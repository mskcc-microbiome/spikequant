...
```{r}
system("scp lilac:/data/brinkvd/users/watersn/spikequant/benchmarking/data_summary.tsv data_summary.tsv")
system("scp lilac:/data/brinkvd/users/watersn/spikequant/benchmarking/results/merged_benchmarking_coverage.tsv merged_benchmarking_coverage.tsv")
...
data_summary_most <- read.csv("data_summary.tsv", sep="\t") %>%
  ...
  summarize(num_seqs = sum(num_seqs)) %>%
  ungroup() %>%
  group_by(sample) %>% 
  mutate(total = sum(num_seqs)) %>%
  ungroup() %>% 
  filter( is_spike)%>% 
  select(sample, Genome, num_seqs, total) %>% 
  distinct()
...
data_summary_nospike <- read.csv("data_summary.tsv", sep="\t") %>% 
  filter(grepl("spike0.00000", file)) %>% 
    filter(!grepl("tmp.*fq", file)) %>% 
  mutate(
    num_seqs=as.numeric(gsub(",", "", num_seqs)),
    sample=gsub("(\\d+_depth.*?_spike.*?)_.*", "\\1", file),
  ) %>% 
 group_by(sample) %>% 
  summarize(total = sum(num_seqs)) %>% ungroup() %>%
  mutate(num_seqs=0) %>% 
  left_join(data.frame(num_seqs=rep(0, length(spikes)), Genome=spikes), relationship= "many-to-many")
...
tmp <- read.csv("merged_benchmarking_coverage.tsv", sep="\t")  %>% 
  ...
  select(sample, dbtype, quantref, aligner, is_spike, target, Genome, Length, genome_base, full_genome, Read_Count, counts, num_seqs, total) %>% 
  mutate(counts_perc = num_seqs/counts)


View(tmp %>% filter(is_spike) %>% filter(grepl("1_depth10000000_spike0.00000", sample), aligner == "bwa-mem", dbtype=="bins"))
# TODO: recalculate abundance for whole spikes based off primary only
...
ggplot(tmp %>% filter(is_spike), aes(y=interaction(target, dbtype), x=counts_perc, color=Genome)) +
  geom_jitter(width=0) + 
  facet_grid( aligner+quantref ~  target, scales="free", space = "free") + 
