...
dir.create("benchmarking/v4")
system("scp lilac:/data/brinkvd/users/watersn/spikequant/benchmarking/v4/all_coverage.tsv benchmarking/v4/all_coverage.tsv")
res4 = read.csv("benchmarking/v4/all_coverage.tsv" , sep="\t", header=TRUE, col.names = c("rawsample", "spike", "len", "nreads") )%>% 
...
ggplot(res4, aes(x=quantbed, y=count_percent)) +
...
res4 %>% 
  filter(aligner == "bwa-mem") %>% 
  inner_join(data_heat_summary_v4) %>%
